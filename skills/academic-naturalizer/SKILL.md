...
If diagnostics are requested, separate them from the manuscript and report:
- formulaic patterns found;
- logic or terminology risks;
- major edits made;
- any passage that cannot be safely revised without more evidence.

## Provenance

This skill is a compact original adaptation informed by the MIT-licensed `academic-humanizer` skill from `dongshuyan/compass-skills` and the general writing-quality approach of `avoid-ai-writing`. It is intentionally focused on prose quality rather than detector evasion.
